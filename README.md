...
A Google Apps Script tool that aggregates media signals from multiple sources and generates daily narrative summaries using Gemini.
...
Communications and insights teams often need to track media coverage across multiple channels. Manual monitoring is time-consuming and inconsistent. This tool automates the collection and synthesis, surfacing dominant themes and emerging narratives without requiring constant attention.
...
The script calls Gemini (gemini-2.5-flash) to generate a structured daily summary. The prompt is designed to be grounded and conservative:
...
If no new entries are found, the run completes silently.
...
- Additional API sources (Reddit, Twitter/X, news APIs)
- Configurable summary frequency (weekly rollups)
